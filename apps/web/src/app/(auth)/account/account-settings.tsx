...
export function AccountSettings() {
  ...
  const deletionBlockers = api.account.deletionBlockers.useQuery();
    ...
    <div className="mx-auto grid max-w-3xl gap-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Your email is {email}. Email changes require a verified-email
            delivery flow and are not currently available.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          {hasCredential ? (
            ...
            <p className="text-muted-foreground text-sm">
              This account uses a social sign-in and has no password to change.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid gap-3">
          {sessions.map((userSession) => {
            const isCurrent = userSession.token === session.session.token;
            return (
              <div
                className="flex items-center justify-between gap-4 rounded-lg border p-3"
              ...
              >
                ...
                {!isCurrent ? (
                  <Button
                    disabled={isBusy}
                    onClick={() => void revokeSession(userSession.token)}
                    type="button"
                    variant="outline"
                  >
                    Sign out
      ...
      <Card className="ring-destructive/30">
        <CardHeader>
          ...
          <CardDescription>
            Permanently removes your profile, enrollments, learning progress,
            assessment attempts, sessions, and sign-in methods. This cannot be
            undone.
          </CardDescription>
        </CardHeader>
        <CardContent>
