...
type ThemeColorRole = "accent" | "background" | "foreground";
...
export function createThemeColorPair(
  ...
  const light = normalizeHexColor(baseColor);
  if (!light) throw new Error(`Invalid theme color: ${baseColor}`);

  const { hue, saturation, lightness } = hexToHsl(light);
  let darkLightness: number;
  let darkSaturation = saturation;
  const adjustment = brightnessAdjustment(darkBrightness);

  if (role === "background") {
    const target = clamp(0.11 + adjustment * 0.18, 0.04, 0.24);
    darkLightness = Math.min(lightness, target);
    darkSaturation = Math.min(saturation, 0.5);
  } else if (role === "foreground") {
    const target = clamp(0.84 + adjustment * 0.17, 0.76, 0.96);
    darkLightness = Math.max(lightness, target);
  ...
  return {
    light,
    dark: hslToHex(hue, darkSaturation, darkLightness),
  };
}
...
export function createSidebarColorPair(
  ...
  return {
    ...
    dark: shiftLightness(
      background.dark,
      clamp(0.035 + brightnessAdjustment(darkBrightness) * 0.045, 0.015, 0.07),
    ),
  };
}
...
export function createCardColorPair(
  ...
  return {
    light: shiftLightness(background.light, lightShift),
    dark: shiftLightness(background.dark, 0.025),
  };
}

export { DEFAULT_DARK_BRIGHTNESS, type ThemeColorPair, type ThemeColorRole };
